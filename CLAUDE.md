...
**Stage:** actively developed, not yet launched. Every backend service is implemented (no stubs remain) as of the last full audit; open gaps and deferred work are tracked in `TODO.md`, not here. The microservices migration is tracked in `docs/migration/` (what moved, the cutover, rollback) and a plan doc outside the repo (phase design and reliability strategy) — cutover is done and the architecture/data-model/API docs describe the split; deleting `legacy-monolith` is the remaining step. It is layered on top of the audited-complete backend, not a sign that something regressed.
