...
VibeCraft is a Spring Boot 4.1.0 application (Java 25, Maven), an AI-assisted project-building platform. The JPA entity layer is implemented (`entity`/`enums` packages — see [docs/README.md](docs/schema/README.md#entities--models) for the full schema, currently **v4** — project ownership lives on `ProjectMember.projectRole == OWNER` rather than a `Project.owner` FK, and `User`'s login fields are `username`/`password`, not `email`/`passwordHash`). REST controllers, request/response DTOs, service interfaces, `@Service` implementations, a `repository` layer (3 of 10 entities so far), a MapStruct `mapper` layer (3 of 10 entities so far), a `GlobalExceptionHandler`, and stateless JWT authentication (Spring Security + JJWT, `security` package) all exist (see [docs/README.md](docs/api/README.md#apis) for the endpoint list). `ProjectServiceImpl`, `ProjectMemberServiceImpl`, and `AuthServiceImpl.signup`/`login` are fully implemented; `UserServiceImpl` implements Spring Security's `UserDetailsService` for real but its own `getProfile()` is still a stub; every other service method is still a stub (`null`/empty/no-op). **Known gap, now live:** none of `ProjectServiceImpl`/`ProjectMemberServiceImpl`'s methods check the caller's `ProjectRole` — any authenticated project member can update/delete a project or manage other members' roles, and `updateMemberRole`/`removeProjectMember` don't even check project membership. This used to be moot while every request was hardcoded to `userId = 1L`; now that real JWT auth is wired up, it's a live authorization hole. Don't build further access-control-sensitive features on top of this without restoring an owner check first — see docs/README.md's [Project Status](docs/project-status.md#project-status) "Known gaps". Also: `AuthServiceImpl.signup` returns the literal string `"dummy"` as the token instead of the JWT it actually generates — a client must call `/api/auth/login` separately to get a usable token.
