...
_Last updated: 2026-04-26 (v3 entity schema: simplified ownership/roles/chat/usage designs, restored JPA annotations after an external revert; added repository, MapStruct mapper, and global exception handling layers; first real service logic in `ProjectServiceImpl`; `application.yaml`/`data.sql` are now committed; `ProjectServiceImpl` completed — `getUserProjectById`, `updateProject`, `softDelete` implemented alongside a new `findAccessibleProjectById` repository query and a `ForbiddenException`/403 handler)_
