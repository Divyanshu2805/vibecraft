...
| GET | `/api/me/subscription` | — | `SubscriptionResponse` | Stub — `SubscriptionServiceImpl.getCurrentSubscription()` still returns `null` |
...
| POST | `/api/payments/portal` | — | `PortalResponse` (`portalUrl`) | Stub — delegates to `PaymentProcessor.openCustomerPortal()`, which still returns `null` |
