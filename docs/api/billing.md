...
| POST | `/webhooks/payment` | raw body + `Stripe-Signature` | 200, or 400 on bad signature | No auth (can't carry a session) — signature-verified instead. A bad signature is 400, not 500, so Stripe doesn't retry something that can never verify. |
