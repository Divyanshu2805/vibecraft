...
- **`ChatEvent`** — the unit of "what the AI did" in one turn (intelligence-service): a thought, a message, a checklist step, a file edit or delete, a teaching-mode walkthrough, or a tool-call log. There is no separate "checkpoint" or file-version concept — see `docs/schema/conventions.md`'s "No checkpoint/rollback system."
