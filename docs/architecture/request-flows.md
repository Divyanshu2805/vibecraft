...
sequenceDiagram
    ...
    AG->>WS: POST /internal/v1/projects/{id}/revisions - the whole turn's changes, one call
    Note over WS: RevisionPublisherImpl: stage blobs -> manifest -> apply -> CAS-publish (AI-05)<br/>all-or-nothing; failure rolls back everything this call applied
...
8. **`finalizeChats`** (in `AiGenerationServiceImpl`) — runs on `Schedulers.boundedElastic()` *after* the stream, off the request thread, so it carries the user id explicitly rather than reading it from a `SecurityContext` that isn't there. Calls `commitFileChanges`, which builds one `PublishRevisionRequest` from the whole turn's `FILE_EDIT`/`FILE_DELETE` events and makes a single `WorkspaceServiceClient.publishRevision` call (CODE_REVIEW.md AI-05 — replacing the old per-file `saveFile`/`deleteFile` loop). Publishing is all-or-nothing: a `FAILED`/`CONFLICT` response reports every changed path as failed, not just the one that broke, and those events are stripped before the turn is persisted — a failed write is never recorded as if it had succeeded. `previousContent` for each edited file's diff comes back on the same response (workspace-service resolves it server-side now), not a separate pre-write read. Then `ChatMessage` + its `ChatEvent` children are saved (`saveAll`, falling back to one-at-a-time if the batch fails — a single bad event shouldn't destroy an otherwise-good conversation record). **A generation that is stopped never reaches `finalizeChats`**: its output is discarded and not billed. See `docs/schema/conventions.md`'s "Revision manifests" for what `publishRevision` actually does inside workspace-service (stage → manifest → apply → CAS-publish, with rollback on any failure).
9. **`llm/AiUsageRecorder`** — records the exchange's token usage into the counter and the ledger.

**Self-correction that does *not* exist yet:** if the generated code fails to install or fails to boot in the live-preview pod, nothing feeds that failure back into another AI turn automatically. The model gets feedback only within *this* request/response cycle (e.g. `looksLikeAbandonedEdit` retrying once if the model narrated an edit but produced zero `FILE_EDIT` events, and the same single retry when the model returned no text at all, after which a turn that is still empty is saved with an explicit "nothing was changed" message instead of a bare "Worked for Ns") — never from an actual runtime/build failure. See `TODO.md`'s "AI prompt/generation reliability improvements" if this is being worked on.
