...
- [ ] **Uptime monitor** on `https://vibecraft.divyanshuagrahari.dev` and `/api/plans`, emailing the owner when either fails.
...
- [ ] **Repo docs:** update `docs/architecture/`, `docs/local-development/` and `CLAUDE.md` with the deploy setup, per the repo's own definition of done. Partly there already: `CLAUDE.md` documents the `deploy/k8s/` topology and its gotchas, and each of the other two has a pointer paragraph (`docs/architecture/` "Deployment topology", `docs/local-development/` on the second kind exercise). Still missing: how the CI/CD pipeline and the secrets flow work, and the live-URL/tunnel setup.
