...
**The old monolith** was removed once the new stack had been used for a while. To inspect or run it, check out the commit before its removal: `git log --diff-filter=D -1 --format=%h -- legacy-monolith/pom.xml` prints the removing commit, and its parent still has the module. Running it against today's data would not work — anything written to the new services' databases since the cutover was never copied back (`docs/migration/`, Phase 4).
