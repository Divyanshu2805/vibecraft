...
- **Live preview cannot start** (503) — pre-existing and reproduced on `legacy-monolith`'s own classpath (`kubernetes-client 6.13.4` × Jackson 2.21.4 NPE when serializing a listed Pod). Fixed in its own session (a JSON merge patch instead of an `update`); see that commit rather than this entry.
...
Sign-out-everywhere and a true revoked-cookie replay (the cookie is `httpOnly`, and both real accounts are in use elsewhere); creating a *new* Stripe checkout session (both real accounts already have a subscription, so the guard refuses first) and `change-plan` (it would modify a subscription); rate-limit 429s; and, until its fix lands, live K8s preview. Residue in the two real accounts from these passes: a few dozen usage events; soft-deleted test projects (Aditi: 134, 137, plus 135/136 earlier; you: 132, 133, created by mistake and removed at once) with their template files still in storage; a copy of project 126's files under the deleted fork.
