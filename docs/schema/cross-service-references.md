...
What that means in practice: a dangling id is possible (a user or project that no longer exists), and nothing in the database prevents it. Usage rows deliberately outlive their project — the insights page attributes tokens spent before a delete, and asks workspace for deleted projects' names for that reason. The one-off migration script reports every dangling id it finds rather than failing on them.
