...
/**
 * Mirrors account-service's own FirebaseConfig - each service verifies the same Firebase session cookie
 * independently (see WebSecurityConfig's javadoc for why this isn't delegated to gateway-service yet).
 */
@Configuration
public class FirebaseConfig {

    /** Named rather than Firebase's DEFAULT app, so a context restart in the same JVM reuses it instead of throwing. */
    private static final String APP_NAME = "vibecraft-intelligence";
