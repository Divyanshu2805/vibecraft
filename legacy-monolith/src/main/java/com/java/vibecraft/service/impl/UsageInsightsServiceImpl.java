...
import com.java.vibecraft.dto.usage.UsageEventResponse;
...
import com.java.vibecraft.util.UsageInsightsAssembler;
...
public class UsageInsightsServiceImpl implements UsageInsightsService {
    ...
    public UsageInsightsResponse getInsights(String range) {
        ...
        int limit = plan != null && plan.getMaxTokensPerDay() != null ? plan.getMaxTokensPerDay() : SubscriptionService.FREE_TIER_DAILY_TOKENS;
    ...
    private Map<Long, ProjectInfo> projectInfo(Set<Long> ids) {
        ...
        for (Project project : projectRepository.findAllById(wanted)) {
            info.put(project.getId(), new ProjectInfo(project.getName(), project.getDeletedAt() != null));
        }
        return info;
    }
