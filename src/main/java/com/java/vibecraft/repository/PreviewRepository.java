...
public interface PreviewRepository extends JpaRepository<Preview, Long> {
    ...
    @Transactional
    @Query("UPDATE Preview p SET p.detail = :detail WHERE p.id = :id AND p.status = com.java.vibecraft.enums.PreviewStatus.CREATING")
    int updatePhase(@Param("id") Long id, @Param("detail") String detail);

    @Modifying(clearAutomatically = true)
    @Transactional
    @Query("""
            UPDATE Preview p SET p.status = com.java.vibecraft.enums.PreviewStatus.RUNNING, p.detail = null,
                   p.readyAt = :now, p.lastAccessedAt = :now
            WHERE p.id = :id AND p.status = com.java.vibecraft.enums.PreviewStatus.CREATING
            """)
    ...
    @Transactional
    @Query("""
            UPDATE Preview p SET p.status = com.java.vibecraft.enums.PreviewStatus.CREATING, p.detail = :detail,
                   p.readyAt = null, p.lastAccessedAt = :now
            WHERE p.id = :id AND p.status = com.java.vibecraft.enums.PreviewStatus.RUNNING
            """)
    ...
    @Transactional
    @Query("""
            UPDATE Preview p SET p.status = com.java.vibecraft.enums.PreviewStatus.FAILED, p.detail = :detail,
    ...
    @Transactional
    @Query("""
            UPDATE Preview p SET p.status = com.java.vibecraft.enums.PreviewStatus.TERMINATED, p.detail = :detail,
                   p.terminatedAt = :now
            WHERE p.id = :id AND p.status IN (com.java.vibecraft.enums.PreviewStatus.CREATING,
                                             com.java.vibecraft.enums.PreviewStatus.RUNNING)
            """)
