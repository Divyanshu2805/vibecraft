...
public class SessionServiceImpl implements SessionService {
    ...
    void checkSignInIsUsable(VerifiedIdentity identity) {
        ...
        // Firebase marks Google accounts verified itself; a password account has to click its verification link. An
        // unverified address could belong to anyone, so it can't be allowed to claim or create an account.
    ...
    AccountResolution resolveAccount(VerifiedIdentity identity) {
        ...
        if (byEmail.isPresent()) {
            ...
            if (user.getFirebaseUid() != null) {
                // Firebase allows one account per email, so this means the Firebase account was deleted and recreated.
        ...
        User created = userRepository.save(User.builder()
                ...
                // The column is NOT NULL and these accounts have no local password - Firebase holds the credential. A
                // hash of 256 random bits nobody has seen can never be signed in with on the legacy path.
